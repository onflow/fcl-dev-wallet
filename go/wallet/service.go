...
func serviceHandler(server *server) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodOptions {
			...
			pollingId := len(server.pollingSessions)
			...
			newPollingSessions := append(server.pollingSessions, string(pendingResponseJson))
			server.pollingSessions = newPollingSessions
